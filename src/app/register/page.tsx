...
import { UserPlus, ShieldAlert, Coins } from "lucide-react";
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-[90vh] flex items-center justify-center p-4 py-16">
      <Card className="w-full max-w-lg border-4 border-accent bg-card/50 backdrop-blur-sm shadow-2xl">
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-[10px] font-mono uppercase tracking-widest text-accent font-bold">真实姓名</label>
                <Input placeholder="张三" className="bg-background/50 border-accent/20" required />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-mono uppercase tracking-widest text-accent font-bold">绰号</label>
                <Input placeholder="韭菜一号" className="bg-background/50 border-accent/20" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-mono uppercase tracking-widest text-accent font-bold">常用邮箱 (用于接收欠费通知)</label>
              <Input type="email" placeholder="victim@example.com" className="bg-background/50 border-accent/20" required />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-mono uppercase tracking-widest text-accent font-bold">资产证明 (余额截图)</label>
            ...
            <Button disabled={loading} className="w-full bg-accent hover:bg-accent/80 text-black font-black h-14 text-lg shadow-lg">
              {loading ? "正在生成受骗证书..." : "立即入坑"}
            </Button>
          </form>
          
          <div className="mt-8 pt-6 border-t border-accent/10 flex flex-col gap-4 text-center">
            <Link href="/login" className="text-xs text-primary font-bold hover:underline flex items-center justify-center gap-1">
